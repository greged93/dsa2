...
def log4(*s):
  if verbosity >= 4 : print(*s, flush=True)
...
def test_hdfs():        
    ...
    df1 =  pa_read_file(path=  'hdfs:///user/scoupon/nono/quantity_extract/ztest_1k.parquet', 
                        cols= cols, verbose=1, n_rows=1000) 
    print('loaded', df1.head(2), df1.shape)
    
    ### Write back on disk
    pa_write_file(df1, hdfs_path=  'hdfs:///user/scoupon/nono/quantity_extract/ztest_1k.parquet/', 
                  cols=['shop_id', 'item_id'], n_rows=1000, partition_cols=None,  verbose=1)
                  
                              
def pa_read_file(path= 'local_or_hdfs' , 
                 cols=None, n_rows=1000, file_start=0, file_end=100000, verbose=1, ) :
    """
      'hdfs://user/scoupon/nono/quantity_extract/ichiba_m_item_aog_export_clean_v01.parquet/'
    ...
    for pfile in flist:
        ...
        dfall = pd.concat((dfall, df)) if dfall is not None else df
        del df
        if len(dfall) > n_rows :
            break
...
def pa_write_file(df, path= 'local_or_hdfs'  , 
...
def pd_read_file(path_glob="*.pkl", ignore_index=True,  cols=None, verbose=False, nrows=-1, concat_sort=True, n_pool=1, drop_duplicates=None, col_filter=None, col_filter_val=None,  **kw):
  """
...
def glob_filter(path: str, filter_ext: list=None, filter_name: list=None):
    import glob, ntpath
    from pathlib import Path
    flist = glob.glob( ntpath.dirname(path)+"/*" ) # ntpath.dirname(path_data_x)+"/*"
    flist = [ f for f in flist if Path(f).suffix.strip().lower() in filter_ext  ]
    flist = [ f for f in flist if filter_name[0] in f.split("/")[-1]  ]  ## Name filtering
...
def pd_dtype_reduce(dfm, int0 ='int32', float0 = 'float32') :
    ...
    for c in dfm.columns :
        if   dfm[c].dtype ==  np.dtype(np.int64) :     dfm[c] = dfm[c].astype( int0 )
        elif dfm[c].dtype ==  np.dtype(np.int32) :     dfm[c] = dfm[c].astype( int0 )
...
def glob_str(path_data_x, prefix='features', verbose=True):
    import glob
    if "*" in path_data_x.split("/")[-1] :
        fstr = path_data_x
    
    elif ".parquet" in path_data_x.split("/")[-1] :
        fstr = path_data_x        
    
    else :
        fstr = os.path.abspath(path_data_x) + f"/{prefix}*.parquet" 
    if verbose : log( glob.glob(fstr) ) 
    return fstr


def load_dataset(path_data_x, path_data_y='',  colid="jobId", n_sample=-1, path_local_in=None, cols=None,
                 shuffle=True, verbose=True):
    """ return a datraframe:  https://raw.github.com/someguy/brilliant/master/somefile.txt
    :param path_data_x:
    :param path_data_y:
    :param colid:
    :param n_sample:
    :return:
    """
    log('loading', colid, path_data_x, path_data_y)
    import glob, ntpath
    cols = None
    # supported_extensions = [ ".txt", ".csv", ".zip", ".gzip", ".pkl", ".parquet" ]
    supported_extensions = [  ".parquet" ]

    path_local           = os.path.join(os.path.curdir, "/data/input/local/") if path_local_in is None else path_local_in
    if path_data_x.startswith("http"):
        path_local   = path_local + "/http/"
        path_data_x  = dataset_fetch_http(path_data_x, path_local)

    if path_data_x.startswith("hdfs"):
        df = dataset_fetch_hdfs(path_data_x, path_data_y,  colid=colid,  n_sample= n_sample, path_local_in=path_local_in)
        return df

    log("#### Load dfX features ############################################")
    #flist = glob_filter(path_data_x, filter_ext = supported_extensions, filter_name= ['features'] )
    fstr = glob_str(path_data_x, prefix='features', verbose=True)           

    from utilmy import pd_read_file
    df   = pd_read_file(fstr, cols=cols, verbose=verbose, n_pool=3) 
    df   = pd_dtype_reduce(df)
    assert len(df) > 0 , " Dataframe is empty: " + path_data_x    
    
    #### Custom Filter #######################################################
    log('Before Filtered', df.shape)
    if  'total_imp_cnt' in df.columns :
        df = df[ df['total_imp_cnt'] > 0.05 ]
        log('After Filtered', df.shape)
    
    #if  'total_no_of_clicks' in df.columns :
    #    df = df[ df['total_no_of_clicks'] > 0.1 ]
    #    log('After Filtered', df.shape)
    
        
    #### Shuffle, Sampling, unique column_id  ################################
    if shuffle                 : df = df.sample(frac=1.0)
    if n_sample > 0            : df        = df.iloc[:n_sample, :]
    if colid not in df.columns : df[colid] = np.arange(0, len(df))
    
    df = df.set_index(colid)
    log("dfX: ", df.head(4), df.columns, df.shape)


    log("###### Load dfy target values #####################################")
    try:  
        if (path_data_y.startswith("http")):
           path_data_y = dataset_fetch_http(path_data_y, path_local)
        
        else :
            path_data_y = "/a/acb401/ipsvols06/ndata/fraud/input/features/202011/ip_yfraud_20210431/"

        # flist = glob_filter(path_data_y, filter_ext = supported_extensions, filter_name= ['target'] )
        fstr = glob_str(path_data_y, prefix='target', verbose=True)  
        dfy  = pd_read_file(fstr)
                
        if colid not in list(dfy.columns) :
            dfy[colid] = np.arange(0, len(dfy))

        log("dfy", dfy, dfy.columns)
        dfy = dfy.set_index(colid)
        log('dfy stats', dfy.mean() )
        df = df.join(dfy, on=colid, how='left', )

        # if len(dfy) < 1:  ### Default value
        log('df[y] set to zero')
        df['y'] = 0 
...
def dataset_fetch_hdfs(path_data_x, path_data_y='',  colid="jobId", n_sample=-1, path_local_in=None, mode='pyarrow'):
    """Fetch dataset from HDFS
    """
    log("###### Fetch in local #######################################")
    
    if mode == 'copy_local' :
        ### Direct read from HDFS
        path_local_x = path_local_in +'/dfX/'
        cmd = f"hadoop dfs -copyToLocal {path_data_x} {path_local_x} "
        os.system(cmd)

        path_local_y = path_local_in +'/dfy/'        
        cmd = f"hadoop dfs -copyToLocal {path_data_y} {path_local_y} "
        os.system(cmd)

        df = load_dataset(path_local_x, path_local_y,  colid, n_sample, path_local_in)
        return df


    if mode == 'pyarrow' :
        ### Direct read from HDFS
        df = pa_read_file(path=  path_data_x, 
                          cols=None, n_rows=10000000, file_start=0, file_end=100000, verbose=1, ) 
        ...
        log("###### Load dfy target values #####################################")
        try:
            dfy = pa_read_file(path=  path_data_y, 
                               cols=None, n_rows=10000000, file_start=0, file_end=100000, verbose=1, ) 
...
def dataset_fetch_http(url_dataset, path_target=None, file_target=None):
...
def load_function_uri(uri_name="myfolder/myfile.py::myFunction"):
    ...
    try:
        ...
        try:
            ...
            log("adding path", path_parent)

            #### import Absolute Path model_tf.1_lstm
            model_name   = Path(package).stem  # remove .py
            package_name = str(Path(package).parts[-2]) + "." + str(model_name)
            log("loading : ", package_name, name)
        ...
        for t in pars['pipe_list'] :
            try:
                ...
                if t['name'] == 'quantile_cutoff' : 
                  s1 = df[x].quantile(0.95)
                  s0 = df[x].quantile(0.05)
                  # me = df[x].median()
                  df[x]  = (df[x] - s0 )/ ( s1-s0 )
...
def pd_stat_distribution_colnum(df, nrows=2000, verbose=False):
    """ Stats the tables
    """
    df = df.sample(n=nrows, replace=True)
    coldes = ["col", "coltype",  "count", "max", "min",  "median", "mean", 
