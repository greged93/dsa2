...
"""
import warnings,sys, os, json, importlib, copy, glob
...
def predict(model_dict, dfX, cols_family, post_process_fun=None):
    ...
    # coly        = load(model_path + "/coly.pkl")   ## column name
    # assert coly        is not None, "cannot load colsx, " + model_path
    coly   = data_pars['coly']
    log2(colsX, coly)
...
def run_predict(config_name, config_path, n_sample=-1,
                path_data=None, path_output=None, pars={}, model_dict=None, return_mode='file,sample'):
    ...
    f_output = "pred"
    if "*" in path_output.split("/")[-1] :  ### Multi file output
        f_output    = path_output.split("/")[-1].replace("*", "")
    ...
    log(path_output)    
    if 'full' in compute_pars.get('pred_export_pars', ''):
       dfX.to_parquet(f"{path_output}/{f_output}_all.parquet")
    
    #### Pred only
    colexport = [cols['colid'], cols['coly'] + "_pred"]
    if cols['coly'] + '_proba' in  dfX.columns :
        colexport.append( cols['coly'] + '_proba' )
    dfX[ colexport ].to_parquet(f"{path_output}/{f_output}.parquet")

    if 'sample' in return_mode or 'sample' in compute_pars.get('pred_export_pars', '') :
        dfX.sample(n=500, replace=True).to_csv(f"{path_output}/sample_{f_output}_all.csv")
        # dfX[ colexport ].sample(n=100, replace=True).to_csv(f"{path_output}/{f_output}_sample.csv")
